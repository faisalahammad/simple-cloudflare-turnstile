...
- New: The IP whitelist now supports CIDR ranges (IPv4 and IPv6), e.g. 203.0.113.0/24 or 2001:db8::/32, so visitors with a dynamic or dual-stack address can be whitelisted by subnet instead of individual IP.
- Fix: IP whitelist matching now normalizes addresses, so an IPv6 address written in a different format still matches, and a visitor reported as an IPv4-mapped IPv6 address (e.g. ::ffff:203.0.113.5) still matches an IPv4 entry or range. A /0 range such as 0.0.0.0/0 is now ignored, as it would disable Turnstile for every visitor.
